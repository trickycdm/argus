...
- **Schema changes are additive-only.** One JSON object per line: `v, ts, event, detail, session_id, cwd, transcript, iterm, ppid`. Old app versions must keep decoding new hook output and vice versa — bump `v` and add nullable fields; never rename or repurpose one.
...
1. Smoke the hook with hostile input and validate the output line:
   `printf '%s' '{"session_id":"t","cwd":"/tmp/has \" quote","transcript_path":"","message":"say \"hi\" \\ done"}' | ./hooks/argus-hook.sh Notification` then pipe the appended line through `python3 -m json.tool`. Use `printf '%s'`, never `echo` — zsh's `echo` interprets backslash escapes and corrupts the test input before the hook sees it. Remove test lines from the live log afterwards.
