...
- **Notification banners draw the icon Launch Services has registered for the bundle id, and a stale record beats the bundle on disk.** An icon that changed after the app first registered renders as a frosted placeholder in banners while Finder, the Dock, and `NSWorkspace.icon(forFile:)` all show it correctly — so verify icon work from a real banner, never from Finder. `bundle.sh` drops and re-adds the LS record (`lsregister -u` then `-f`) for this reason; `-f` alone refreshes the path but not the banner icon. If a banner is still stale, restart the notification daemons (`killall usernoted NotificationCenter` — launchd respawns both) and relaunch the app. The committed `.icns` is sufficient on macOS 26: no `Assets.car`/`CFBundleIconName`, and therefore no `actool`/full-Xcode dependency.
...
2. Notification paths: only testable from the bundled app (grant the prompt via onboarding's ENABLE button, or at launch once onboarding is recorded). To force a banner on demand, append a synthetic `SessionStart` + `Notification` pair for an unused session id to today's `~/Library/Application Support/Argus/events-<YYYYMMDD>.jsonl` (`detail` must satisfy `SessionStore.notificationNeedsYou`; use `ppid` 0 so no live process is adopted and the row retires on transcript staleness, and any `source` but `fork`). Check the banner's icon here. Remove the lines afterwards **while the app is stopped** — shrinking the log under a running app resets the tailer's offset and replays the day non-quietly, re-firing notifications.
